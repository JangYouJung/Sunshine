...
router.post("/", function (req, res) {
  ...
  connection.query(
    ...
    function (err, rows) {
      ...
      if (valid) {
        // 이미 유효한 정보가 있으면 이전 화면으로 redirect
        res.write(
          "<script type= 'text/javascript'>alert('This course has valid attendance!');</script>"
        );
        res.write(
          "<script type='text/javascript'>location.href='staff_main'</script>"
        );
      } else {
        //새로운 출석 정보 생성
        ...
        connection.query(
          ...
          function (err2, rows2) {
            ...
            async function GenSQLInsert() {
              //SQL Insert문을 동기적으로 실행되도록 하는 함수
              //[1] async, await 사용하여 차시 속성 먼저 받아오기
              let promise = new Promise((resolve, reject) => {
                if (exist) {
                  //(1)DB에서 마지막 차시를 불러와 다음 차시 번호 구성
                  connection.query(
                  ...
                  setTimeout(function () {
                    ...
                    connection.query(
                      ...
                      function (err4, rows4) {
                        ...
                        res.write(
                          "<script type= 'text/javascript'>alert('attendance finish');</script>"
                        );
                        res.write(
                          "<script type='text/javascript'>location.href='staff_main'</script>"
                        );
